...
static void button_handler(BUTTON_EVENT event, bool is_called_from_isr)
{
  ...
  else if (BTN_EVENT_SHORT_PRESS(BOARD_BUTTON_PBEXT1) == event)
  {
    app_event = EVENT_APP_TOGGLE_LEDEXT1;
  }
...
void app_hw_init(void)
{
  ...
  Board_EnableButton(BOARD_BUTTON_PBEXT1);
...
void cc_binary_switch_handler(cc_binary_switch_t * p_switch)
{
  ...
  switch (p_switch->endpoint) {
    case 1:
      ...
      break;
    case 2:
      Board_SetLed(BOARD_LEDEXT1, (value > 0) ? LED_ON : LED_OFF);
      break;
  }
}
