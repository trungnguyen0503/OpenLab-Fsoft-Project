...
zaf_event_distributor_app_event_manager(uint8_t event)
{
  ...
  switch (event)
  {
    case EVENT_APP_TOGGLE_LED:
      p_switches = cc_binary_switch_get_config() + 0;
      if (cc_binary_switch_get_current_value(p_switches) > 0)
        {
          cc_binary_switch_set(p_switches, 0);
        } 
      else 
        {
          cc_binary_switch_set(p_switches, 0xFF); // 0xFF for last on value.
        }
      break;
    
    case EVENT_APP_TOGGLE_LEDEXT1:
      p_switches = cc_binary_switch_get_config() + 1;
      if (cc_binary_switch_get_current_value(p_switches) > 0)
        {
          cc_binary_switch_set(p_switches, 0);
        }
